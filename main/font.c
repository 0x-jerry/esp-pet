...
// 6x8 ASCII font, characters 0x20 (space) through 0x7E (~)
// Each character is 6 bytes (one per column), LSB at top
static const uint8_t font_data[][6] = {
    // 0x20 space
...
const uint8_t *font_get_char(char c) {
    if (c < 0x20 || c > 0x7E) {
        return font_data[0]; // Return space for unsupported chars
    }
